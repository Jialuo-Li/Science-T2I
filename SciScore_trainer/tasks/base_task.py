...
class BaseTaskConfig:
    limit_examples_to_wandb: int = 1000
    pass
...
class BaseTask:
    ...
    def log_to_wandb(self, eval_dict, table_name="test_predictions"):
        ...
        is_correct = predictions[columns.index('is_correct')]
        indices = [i for i, pred in enumerate(is_correct) if not pred]
        predictions = [[p[i] for i in indices] for p in predictions]
